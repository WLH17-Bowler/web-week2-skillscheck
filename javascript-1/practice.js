...
const doubled = myNumbers.map((element, index) => element * 2)

//////////////////PROBLEM 15////////////////////

// Now, use .filter to create a new array containing any numbers that are greater than 100. Call the new array 'filtered'. 

const filtered = myNumbers.filter((element, index) => element > 100 ? element : null)
...
let total = myNumbers.reduce((accumulator, current, myNumbers) => accumulator + current)
...
let notGeorge = ['Louis', 'Ted', 'Bill', 'Sharon', 'Mark', 'Angela']
...
const enemies = people.filter((element, index, arr) => element.friend === false ? element : null)
console.log(enemies)
...
// Store that value in a variable named 'totallyAwesome'.
// Use .reduce()
// let bobsTotal = purchases.reduce((accumulator, obj) => obj.owner === 'Bob' ? obj.price + accumulator : accumulator, 0)

let totallyAwesome = people.reduce((acc, curr, people) => acc + curr.awesomeLevel, 0)